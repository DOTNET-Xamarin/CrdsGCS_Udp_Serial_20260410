...
namespace CrdsView.Ucs.Setups.Calibrations
{
    public partial class UcCalibration_Compass : UserControl
    {
            ...
            if (message.IsValid)
            {
                try
                {
                    ...
                    sb.AppendLine($"attempt : {message.attempt}, cal_mask : {message.cal_mask}, " +
                        $"cal_status : {((CrdsModel.Mavlink.Ardupilotmega.MavEnums.MAG_CAL_STATUS)message.cal_status).ToString()}");
        ...
        private void Start_Calibration()
        {
            ...
            // command id 42424
            InterfaceController.Instance.GetCurrentVehicle().SetDataStream(10, (byte)CrdsModel.Mavlink.MavEnums.MAV_DATA_STREAM.MAV_DATA_STREAM_EXTRA3);
        ...
        private void Stop_Calibration(bool isUser = true)
        {
            ...
            switch (r)
            {
                ...
                case DialogResult.Yes:
                    // write current value.
                    ack = InterfaceController.Instance.GetCurrentVehicle().SendCommandLong((ushort)CrdsModel.Mavlink.Ardupilotmega.MavEnums.MAV_CMD.MAV_CMD_DO_ACCEPT_MAG_CAL, 0, 0, 0, 0, 0, 0, 0);
                    break;
                case DialogResult.No:
                    // just stop.
                    ack = InterfaceController.Instance.GetCurrentVehicle().SendCommandLong((ushort)CrdsModel.Mavlink.Ardupilotmega.MavEnums.MAV_CMD.MAV_CMD_DO_CANCEL_MAG_CAL, 0, 0, 0, 0, 0, 0, 0);
                    break;
            }
