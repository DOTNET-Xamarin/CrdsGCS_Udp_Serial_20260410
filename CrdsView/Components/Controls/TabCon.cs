...
namespace MyControl
{
    ...
    {
        int ControlWidth = 0;       //control width
        int ControlHeight = 0;      //control height
        int ControlPenWidth = 1;    //control pen width
        int LeftEdge ;
        int RightEdge ;
        int BottomEdge;
        int TopEdge ;
        ...
        SolidBrush b;
        private Font font = new Font("Times New Roman", 8); //Font and size of characters
        Pen p = null;
        ...
        Color ControlBackColor      = Color.Gray;//Control background color
        Color ControlPenColor       = Color.White;
        Color ControlBrushColor     = Color.Red;
        Color ControlMiddleFocus    = Color.Black;
        Color ControlSkyColor       = Color.DodgerBlue; //.DeepSkyBlue; //.RoyalBlue;
        Color ControlGroundColor    = Color.SaddleBrown;// .DarkGoldenrod; // Peru;
        Color ControlLineColor      = Color.White;
        Color ControlCharColor      = Color.Black;
        Color ControlTriangle       = Color.LightYellow;
        Color ControlYawBack        = Color.White;
        Color ColorDisConnect       = Color.Red;
        Color ColorConnect          = Color.GreenYellow;


        Bitmap ControlBitamp        = null;     //Declare a Bitmap in memory as a buffer
        Graphics ControlGraphics    = null;     //Canvas for memory bitmap

        Bitmap RollPitchBitmap      = null;     //Angle cache for pitch and roll
        Graphics RollPitchGraphics  = null;      //angular cached canvas


        bool isMultiaxial = false;       //Whether the aircraft is multi-axis
        ...
        public MyTabCon()
        {
            ...
            LeftEdge = (int)(0.18 * ControlWidth);
            RightEdge = (int)(0.18 * ControlWidth);
            BottomEdge = (int)(0 * ControlHeight);
            TopEdge = (int)(0.13 * ControlHeight);

            //Initialize the memory control canvas
        ...
        void DrawNumberOfSatellites() {
            ...
            {
                //Satellitesbitmap 
                //String str = "Number of satellites:0";
            ...
            Font font1 = new Font("Times New Roman", 12,FontStyle.Bold);
        ...
        private void DrawRollPitchState() {
            ...
            RollPitchGraphics.FillPie(b, 0, 0, 2 * r, 2 * r, RadianPitch, 2 * (90-RadianPitch));// fill arc
            ...
            //draw line up
            for (int i = 0; (0.70f * r < Y - i * LineDistance) && (AngleDistance * i<=90) ; i++)
            {
            ...
            //draw line down
            for (int i = 0; (1.60 * r > Y + i * LineDistance) && (AngleDistance * i <= 90) ; i++)
            {
                if (Y + i * LineDistance < 0.70 * r)
                {
                    continue;
                }
        ...
        private void DrawWarnning() {
            ...
            Font f = new Font("Times New Roman", 0.1f * (RollPitchWidth > RollPitchHeight ? RollPitchHeight : RollPitchWidth));
        ...
        void DrawYawState()
        {
            ...
            SolidBrush b = new SolidBrush(Color.FromArgb(255, Color.LightSkyBlue));
            //this.BackColor = Color.RoyalBlue;
            BitmapGraphics.FillRectangle(b, new Rectangle(0, 0, 2 * ControlWidth, (int)YawHight + 1));

           // Font font = new Font("Times New Roman", (int)(0.03 * this.ControlWidth));

         //   Font font = new Font(fCollection.Families[0].Name, (int)(0.03 * this.ControlWidth));
            Font font = new Font("Verdana", (int)(0.023 * this.ControlWidth));

            float everyscale = 30;//Degrees represented by each tick
            float middlewidth = 0.1f * ControlWidth;
            //width == 0--360 

            float resolution = ((float)this.ControlWidth) / 360.0f;//Pixels needed per degree

            //Calculate the position and angle to start drawing the line
            ...
            Font f = new System.Drawing.Font("Times New Roman", 0.14f * (middlewidth > YawHight ? YawHight : middlewidth) * (6 - 0.8f * yaw.ToString().Length), FontStyle.Bold);
        ...
        void DrawSpeed()
        {
            ...
            Font font = new System.Drawing.Font("Times New Riman", 8,FontStyle.Bold);
            ...
            SolidBrush backBrush = new SolidBrush(Color.FromArgb(255, Color.Black)); 
            ...
            float resolution = 0.25f;//Velocity represented by each pixel
            float startalt = speed - speedHeigh / 2 * resolution;
            float endalt = speed + speedHeigh / 2 * resolution;
            float everyscale = 5;//The speed represented by each tick
            ...
            for (int i = 0; speedHeigh / 2 > (everyscale / resolution) * i; i++)
            {
                if (i % 5 == 0)
                {
                    ...
                    SpeedGraphics.DrawString(str, font, lineBrush, new PointF(speedWidth - longlenght - size.Width * 1.0f, speedHeigh / 2 - (everyscale / resolution) * i - size.Height / 2));
            ...
            SolidBrush b1 = new SolidBrush(Color.FromArgb(120, Color.YellowGreen));
       ...
       void DrawGPS()
       {
           Font font = new Font("Times New Roman", 0.04f * (this.Width > this.Height ? this.Height : this.Width));
        ...
        void DrawVoltageCurrent() {
            ...
            Font f = new System.Drawing.Font("Times New Roman", 0.04f * (this.Width - this.LeftEdge - RightEdge));
        ...
        private void Control_paint(object sender, PaintEventArgs e)
        {
            ...
            Font f=new System.Drawing.Font("Times New Roman",0.03f*(this.Width-this.LeftEdge-RightEdge));
        ...
        /// <summary>
        /// Set whether the current aircraft is multi-axis or fixed wing, the system defaults to multi-axis
        /// </summary>
        /// <param name="isMu">true : Indicates that the system is multi-axis，false : Indicates that the system is fixed wing</param>
