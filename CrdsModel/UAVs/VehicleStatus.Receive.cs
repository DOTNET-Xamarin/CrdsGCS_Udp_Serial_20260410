...
namespace CrdsModel.UAVs
{
    public partial class VehicleStatus
    {
        public object Receive_Vehicledatas(Packet packet)
        {
            ...
            //Console.WriteLine("Rx MSGID : " + packet.MSGID.ToString()); //20251016 - inserted test code
            ...
            switch (packet.MSGID)
            {
                ...
                case 24:
                    ...
                    break;
                case 30:
                    this.Receive_Attitude(packet.Message);
                    break;
                case 34:
                    //Console.WriteLine("MSG ID(65) : RC_CHANNELS_SCALED received."); //20260126 - inserted test code
                    break;
                case 35:
                    this.Receive_RC_CHANNELS_RAW(packet.Message);
                    //Console.WriteLine("MSG ID(35) : RC_CHANNELS_RAW received."); //20260126 - inserted test code
                    break;
                case 65:
                    this.Receive_RC_CHANNELS(packet.Message);
                    //Console.WriteLine("MSG ID(65) : RC_CHANNELS received."); //20260126 - inserted test code
                    break;
                case 74:
                    /*
                    //BOB - 20250930 - test code to check message frame
                    if (packet.Message != null)
                    {
                        string hexString = BitConverter.ToString(packet.Message.Buffer).Replace("-", "");
                        Console.WriteLine("MSG ID(74) : " + hexString);
                    }
                    else
                    {
                        Console.WriteLine("MSG ID(74) : null message frame");
                    }
                    //EOB - 20250930 - test code to check message frame
                    */
                    this.Receive_VFR_HUD(packet.Message);
                    break;
                case 77:
                    //Console.WriteLine("MSG ID(77) : COMMAND ACK received."); //20251016 - inserted test code
        ...
        private void Receive_HeartBeat(Message message)
        {
            ...
            try
            {
                ...
                //Console.WriteLine("VehicleStatus: Receive_heartbeat called");
        ...
        private void Receive_Attitude(Message message)
        {
            try
            {
                var attitude = (Message_ATTITUDE)message;

                this.roll = attitude.roll * 180.0 / Math.PI;
                this.pitch = attitude.pitch * 180.0 / Math.PI;
                this.yaw = attitude.yaw * 180.0 / Math.PI;
                this.rollspeed = attitude.rollspeed * 180.0 / Math.PI;
                this.pitchspeed = attitude.pitchspeed * 180.0 / Math.PI;
                this.yawspeed = attitude.yawspeed * 180.0 / Math.PI;

                Console.WriteLine($"roll: {this.roll:F2} pitch: {this.roll:F2} yaw: {this.yaw:F2}");
        ...
        public ushort Chan9_raw { get; set; }
        public ushort Chan10_raw { get; set; }
        public ushort Chan11_raw { get; set; }
        public ushort Chan12_raw { get; set; }
        public ushort Chan13_raw { get; set; }
        public ushort Chan14_raw { get; set; }
        public ushort Chan15_raw { get; set; }
        public ushort Chan16_raw { get; set; }
        ...
        private void Receive_RC_CHANNELS_RAW(Message message)
        {
            try
            {
                ...
                //Console.WriteLine("rc_ch1:" + this.Chan1_raw
                //    + "  rc_ch2:" + this.Chan2_raw
                //    + "  rc_ch3:" + this.Chan3_raw
                //    + "  rc_ch4:" + this.Chan4_raw);
        ...
        private void Receive_RC_CHANNELS(Message message)
        {
            try
            {
                var rc_channels = (Message_RC_CHANNELS)message;

                this.Chan1_raw = rc_channels.chan1_raw;
                this.Chan2_raw = rc_channels.chan2_raw;
                this.Chan3_raw = rc_channels.chan3_raw;
                this.Chan4_raw = rc_channels.chan4_raw;
                this.Chan5_raw = rc_channels.chan5_raw;
                this.Chan6_raw = rc_channels.chan6_raw;
                this.Chan7_raw = rc_channels.chan7_raw;
                this.Chan8_raw = rc_channels.chan8_raw;
                this.Chan9_raw = rc_channels.chan8_raw;
                this.Chan10_raw = rc_channels.chan8_raw;
                this.Chan11_raw = rc_channels.chan8_raw;
                this.Chan12_raw = rc_channels.chan8_raw;
                this.Chan13_raw = rc_channels.chan8_raw;
                this.Chan14_raw = rc_channels.chan8_raw;
                this.Chan15_raw = rc_channels.chan8_raw;
                this.Chan16_raw = rc_channels.chan8_raw;
                this.Rssi = rc_channels.rssi;
